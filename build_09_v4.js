...
const outFile = '09_FLUXAI_NOVO_CLIENTE_ONBOARDING_SEGURO_FINAL_V4.json';

function createSearchRows(id, sheetId, searchColumn, searchValue, x, y) {
    return {
        id,
        module: "google-sheets:filterRows", // EXACT internal name from Make
        version: 2,
...
function createAggregator(id, feederId, x, y) {
    return {
        id,
        module: "builtin:BasicAggregator", // EXACT internal name from Make
        version: 1,
        parameters: { feeder: feederId }, // EXACT property name from Make
        mapper: {},
        metadata: { designer: { x, y } }
    };
}
...
function createUpdateRow(id, sheetId, aggId, values, x, y) {
    return {
        ...
        mapper: {
            ...
            spreadsheetId: "/1-MZBzS2KOZ_pSIR9rVX_mkecdvmQjF9j9q8ahMKqxQE",
            sheetId,
            rowNumber: `{{get(map(${aggId}.array; "rowNumber"); 1)}}`, // real rowNumber map
            includesHeaders: true,
            useColumnHeaders: false,
            values
        },
...
fs.writeFileSync(outFile, JSON.stringify({ name: "09_FLUXAI_NOVO_CLIENTE_ONBOARDING_SEGURO_FINAL_V4", flow: [webhook, mainRouter] }, null, 4));
console.log("FINAL V4 GENERATED!");
