...
const outFile = '09_FLUXAI_NOVO_CLIENTE_ONBOARDING_SEGURO_FINAL_V5.json';

// Read the exact template generated by Make
const templateRaw = fs.readFileSync('09_FLUXAI_NOVO_CLIENTE_ONBOARDING_SEGURO.blueprint (1).json', 'utf8');
const templateData = JSON.parse(templateRaw);
const orphans = templateData.metadata.designer.orphans[0];

const templateSearch = orphans[0];
const templateAgg = orphans[1];
const templateRouter = orphans[2];

function createSearchRows(id, sheetId, searchColumn, searchValue, x, y) {
    const obj = JSON.parse(JSON.stringify(templateSearch));
    obj.id = id;
    obj.parameters = { "__IMTCONN__": null };
    obj.mapper = {
        mode: "select",
        from: "drive",
        ...
        filter: [
            { column: searchColumn, operator: "text:equal", value: searchValue }
        ]
    };
    obj.metadata.designer.x = x;
    obj.metadata.designer.y = y;
    delete obj.metadata.designer.messages; // strip error messages
    return obj;
}
...
function buildSheetFlow(baseId, sheetId, searchValue, values, yBase) {
    ...
    // Overwrite the Placeholder routes with real Add/Update flows
    routerObj.routes = [
        {
            name: "Não existe (Add)",
            filter: {
...
const mainRouter = {
    ...
    version: 1,
    parameters: {},
    mapper: null, // use null as Make does
    metadata: { designer: { x: 150, y: 0 } },
    routes: [r1, r2, r3, r4, r5]
};

fs.writeFileSync(outFile, JSON.stringify({ name: "09_FLUXAI_NOVO_CLIENTE_ONBOARDING_SEGURO_FINAL_V5", flow: [webhook, mainRouter] }, null, 4));
console.log("FINAL V5 GENERATED WITH DEEP TEMPLATE INJECTION!");
