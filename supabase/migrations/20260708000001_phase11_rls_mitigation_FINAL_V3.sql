-- MIGRATION: FASE 11 - MITIGAÇÃO MÍNIMA DE RLS E GOVERNANÇA (VERSÃO FINAL V3)
-- Data: 2026-07-08
-- Limpeza profunda de policies legadas identificadas via pg_policies.
-- Separação de UUID e TEXT para compatibilidade com tabelas PT-BR.
...
CREATE OR REPLACE FUNCTION public.current_user_project_uuid()
RETURNS uuid AS $$
DECLARE
  v_project_id uuid;
BEGIN
...
DECLARE
  v_client_id text;
BEGIN
  -- Fazemos o cast ::text para cobrir o caso em que o campo é UUID e precisamos
  -- comparar com as colunas TEXT das tabelas PT-BR.
  SELECT scoped_project_id::text INTO v_client_id
  FROM public.governance_users
  WHERE email = (auth.jwt() ->> 'email')
  LIMIT 1;
  RETURN v_client_id;
...
CREATE POLICY "Select events Client" ON public.operational_events FOR SELECT TO authenticated USING (
    project_id = public.current_user_project_uuid() 
...
CREATE POLICY "Select projects Client" ON public.projects FOR SELECT TO authenticated USING (id = public.current_user_project_uuid());
...
CREATE POLICY "Client Select PLANEJAMENTO" ON public."PLANEJAMENTO_CONTEUDO" FOR SELECT TO authenticated USING (client_id = public.current_user_client_id());
...
CREATE POLICY "Client Select DEMANDAS" ON public."DEMANDAS_CLIENTES" FOR SELECT TO authenticated USING (client_id = public.current_user_client_id());
...
CREATE POLICY "Client Select CONTRATOS" ON public."CONTRATOS_CLIENTES" FOR SELECT TO authenticated USING (client_id = public.current_user_client_id());
...
CREATE POLICY "Client Select FINANCEIRO" ON public."FINANCEIRO_CLIENTES" FOR SELECT TO authenticated USING (client_id = public.current_user_client_id());
...
CREATE POLICY "Client Select CREDITOS" ON public."IA_CREDITOS_CLIENTE" FOR SELECT TO authenticated USING (client_id = public.current_user_client_id());
...
CREATE POLICY "Client Select EXTRAS" ON public."SERVICOS_EXTRAS_CLIENTES" FOR SELECT TO authenticated USING (client_id = public.current_user_client_id());
...
CREATE POLICY "Client Select ESTRATEGIA" ON public."CLIENTES_ESTRATEGIA" FOR SELECT TO authenticated USING (client_id = public.current_user_client_id());
