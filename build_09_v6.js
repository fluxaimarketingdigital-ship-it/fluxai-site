...
const outFile = '09_FLUXAI_NOVO_CLIENTE_ONBOARDING_SEGURO_FINAL_V6.json';
const templateSearch = {
    id: 1503,
    module: "google-sheets:filterRows",
    version: 2,
    metadata: { designer: { x: -1177, y: 154 } }
};
...
const templateAgg = {
    ...
    version: 1,
    parameters: { feeder: 1503 },
    mapper: {},
    metadata: {
        designer: { x: -877, y: 154 },
        restore: { extra: { feeder: { label: "Google Sheets - Search Rows [1503]" }, target: { label: "Custom" } } }
    }
};
...
const templateRouter = {
    ...
    version: 1,
    mapper: null,
    metadata: { designer: { x: -577, y: 154 } },
    routes: []
};

function createSearchRows(id, sheetId, searchColumn, searchValue, x, y) {
    const obj = JSON.parse(JSON.stringify(templateSearch));
    obj.id = id;
    obj.parameters = { "__IMTCONN__": 8785925 };
    obj.mapper = {
        mode: "select",
        from: "drive",
        ...
        filter: [
            [{ a: searchColumn, o: "text:equal", b: searchValue }]
        ]
    };
    obj.metadata.designer.x = x;
    obj.metadata.designer.y = y;
    delete obj.metadata.designer.messages;
    return obj;
}

function createAggregator(id, feederId, x, y) {
    const obj = JSON.parse(JSON.stringify(templateAgg));
    obj.id = id;
    obj.parameters = { feeder: feederId };
    obj.metadata.designer.x = x;
    obj.metadata.designer.y = y;
    delete obj.metadata.designer.messages;
    if (obj.metadata.restore && obj.metadata.restore.extra && obj.metadata.restore.extra.feeder) {
        obj.metadata.restore.extra.feeder.label = `Google Sheets - Search Rows [${feederId}]`;
    }
    return obj;
}
...
function createUpdateRow(id, sheetId, aggId, valuesUpdate, x, y) {
    return {
        id,
        ...
        mapper: {
            ...
            spreadsheetId: "/1-MZBzS2KOZ_pSIR9rVX_mkecdvmQjF9j9q8ahMKqxQE",
            sheetId,
            rowNumber: `{{get(map(${aggId}.array; "rowNumber"); 1)}}`, 
            includesHeaders: true,
            useColumnHeaders: false,
            ...valuesUpdate
        },
...
function buildSheetFlow(baseId, sheetId, searchValue, valuesAdd, valuesUpdate, yBase) {
    ...
    const routerObj = JSON.parse(JSON.stringify(templateRouter));
    routerObj.id = routerId;
    routerObj.metadata.designer.x = 800;
    routerObj.metadata.designer.y = yBase;
    delete routerObj.metadata.designer.messages;
    ...
    routerObj.routes = [
        {
            ...
            filter: {
                name: "Add",
                conditions: [
                    [ { a: `{{length(${aggId}.array)}}`, o: "numeric:equal", b: "0" } ]
                ]
            },
            flow: [ createAddRow(addId, sheetId, valuesAdd, 1100, yBase - 150) ]
        },
        {
            name: "Existe (Update)",
            filter: {
                name: "Update",
                conditions: [
                    [ { a: `{{length(${aggId}.array)}}`, o: "numeric:greater", b: "0" } ]
                ]
            },
            flow: [ createUpdateRow(updateId, sheetId, aggId, valuesUpdate, 1100, yBase + 150) ]
        }
    ];
...
const s01_upd = { ...s01_add }; delete s01_upd["24"]; // remove data_criacao
...
const s02_upd = { ...s02_add }; delete s02_upd["18"]; // remove data_criacao
...
const r1 = { name: "1", flow: buildSheetFlow(100, "01_CLIENTES_ESTRATEGIA", "{{1.client_id}}", s01_add, s01_upd, -600) };
const r2 = { name: "2", flow: buildSheetFlow(200, "02_CONTRATOS_CLIENTES", "{{1.contrato_id}}", s02_add, s02_upd, -200) };
const r3 = { name: "3", flow: buildSheetFlow(300, "11_DNA_CLIENTE_GPT", "DNA_{{1.client_id}}", s11_add, s11_upd, 200) };
const r4 = { name: "4", flow: buildSheetFlow(400, "10_IA_CREDITOS_CLIENTE", "{{1.limite_id}}", s10_add, s10_upd, 600) };
...
fs.writeFileSync(outFile, JSON.stringify({ name: "09_FLUXAI_NOVO_CLIENTE_ONBOARDING_SEGURO_FINAL_V6", flow: [webhook, mainRouter] }, null, 4));
console.log("FINAL V6 GENERATED WITH EXACT USER REQUIREMENTS!");
